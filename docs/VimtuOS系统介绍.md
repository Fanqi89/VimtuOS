...
| **状态数字（脚本权威）** | **完成 42 / 部分 0 / 未做 1（共 43 项能力）**，唯一未做项是 **Rust 参与实现（可选要求，本机未装工具链）** |
...
| **中文界面与中文文字** | 自带 TrueType 渲染 + **4 个字体面**（西文 Noto Sans / 中文 Noto Sans SC / 终端等宽 Sarasa Mono SC / 缺字兜底 GNU Unifont，均为可再分发开源字体），中文界面与终端显示正常，终端中英严格 1:2 | 像素级断言 + 截图 + `[FONT64]` 四面自检打点 + 与内核光栅化器的逐像素比对 |
| **内核自检** | 每个子系统启动即自检（内存 / 图形 / 调度器 / VFS / store / syscall / ring3 / VAP64 / ELF64 / APIC / SMP / USB / EDID / ACPI），结果打到串口供自动验收 | 各 `[XXX] selftest PASS` |
| **★ 开机滚屏引导控制台（boot console + dmesg）** | 进桌面前把这次启动真的跑出来的内核日志**滚一遍**（黑底 + 终端等宽面 Sarasa Mono + 行首 `[    0.123]` 时间戳，`FAIL/PANIC` 红 / `WARN` 黄）；日志本体是 16 KiB 环形缓冲（头部 16 行永久保留）；终端 `dmesg` 回看，`boot verbose on|off` 持久化开关（下次冷启动直接进桌面）；有界停留 + 任意键跳过 | `bootlog64_test`（30 项：串口 + 像素 + vram 直读滚动证据 + 跳过 + dmesg 早期行 + 冷启动 verbose=0） |
...
### 5.4 开机滚屏引导控制台（批次 N：让启动"看得见"）

在**硬件检查报告页之后、进安装向导 / 桌面之前**，屏上会像 Linux 一样滚一遍这次启动的内核日志：

* **日志从哪来**：`dbg64_putc()` 是串口/调试口输出的**唯一出口**，在它上面挂一个
  `dbg64_set_sink64()` 钩子，内核里既有的一百多处打点**一行都不用改**就镜像进一个
  **16 KiB 环形缓冲**（`kernel/console64.cpp`）：逐字符攒行、每行记下行首 tick，
  行内文本上限 160 字符（超出截断），环形满时丢最旧的行（`dropped` 如实计数）——
  但**头部 16 行单独保留**（长模式 / BootInfo / E820 那些最早的行永远不会被覆盖）。
* **为什么能缓存到最早的行**：缓冲本体用 `section(".data.con64")` 放进 `.data` ——
  `kmain64()` 清 `.bss` 时不会把它抹掉；而 dbg64 的 sink 指针在 `.bss` 里，清完用
  `con64_rehook64()` 重新挂上（顺序写在 `kernel64.cpp` 里）。
* **画法**：黑底 + 终端等宽面逐字符绘制；行首 Linux 风格时间戳 `[    0.123]`（`g_ticks64` 换算，
  宽度 5+6 右对齐）；按关键字着色（`FAIL`/`FAILED`/`PANIC` 红、`WARN` 黄、其余浅灰）；
  行满整体上移一行（像素行拷贝，回放时攒 6 行批量上移，只提交脏区 `fb_flip_region`）。
* **有界停留与跳过**：回放完最近的 320 行后停 1200 tick-ms（约 1 秒级），**按任意键立即结束**
  （按键会被吞掉，不会影响后面的界面），然后继续既有流程（安装介质 → 向导；已装系统 → 桌面）。
* **回看与开关**：终端 `dmesg` 打印「头部保留行 + 省略标记 + 环形缓冲」（串口同时打有界证据
  `[CON64] dmesg[i]`）；`boot verbose on|off` 把 `boot.verbose` 写进 config64/store64，
  关掉后**下次冷启动直接进桌面**（缓冲仍在工作，`dmesg` 照常可用）。

打点（自动验收 grep）：`[CON64] ring init bytes=… lines=… text_max=…` / `screen ready cols=… rows=…` /
`replay lines=… dropped=… ms=…` / `live lines=… drawn=…` / `skip key=1` / `selftest PASS mask=0x0` /
`dmesg lines=…` / `boot verbose=on|off persisted=…`。端到端：`tests/bootlog64_test.py`（30 项断言），
截图 `docs/screenshots/bootlog64.png`。
...
* TrueType 渲染 + 构建期**字体子集化**（**4 套 TTF**：西文 / 中文 / 终端等宽 / 缺字兜底，只把用到的字形打进内核；
字体全部是可再分发的开源字体：Noto Sans、Noto Sans SC、Sarasa Mono SC、GNU Unifont —— 来源、许可与派生方式见 `FONTS.md` 与 `docs/字体许可说明.md`）；
...
```
完成 42 / 部分 0 / 未做 1   （共 43 项能力）
未做项：[工具链] Rust 参与实现（可选要求，本机未装工具链）
全量验收：33 个脚本全部 PASS（本批次新增 tests/bootlog64_test.py 30 条断言；上一批 fileops64_test 95 条仍全过）
```
