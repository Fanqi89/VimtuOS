...
//     * 找不到盘   -> 看"存储"区块：有没有 AHCI/NVMe 控制器与端口、盘型号、容量
...
//   * NVMe（item 6）有**最小驱动**（单控制器/单队列/轮询/512B LBA，见 nvme64.h）；
//     报告里的 NVMe 行是**真实状态**（控制器/命名空间/型号），不是"不支持"。
//   * EHCI / xHCI 只**检测并标注"不支持"**，不做驱动。
...
// ★ item 6 从 34 提到 40：加 NVMe 控制器行 + 每个命名空间一行之后，34 在"AHCI + NVMe + 几块盘"
//   的机器上正好挤满 —— 满了之后 newline() 会覆盖最后一行（报告内容被悄悄吃掉），所以留余量。
#define HWUI64_MAX_LINES   40
